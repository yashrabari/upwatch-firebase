...
import 'package:upwatch/screen/disease_risk_screen/gradiant_pie_chart/degree.dart';
...
class PieChartPainter extends CustomPainter {
  ...
  PieChartPainter(
    ...
    subParts = values;
    prevAngle = degreeToRadian(degreeOptions!.initialAngle);
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (_total == 0) {
      ...
      final isNonGradientElementPresent = (subParts!.length - (gradientList?.length ?? 0)) > 0;

      for (int i = 0; i < subParts!.length; i++) {
        if (isGradientPresent) {
          final endAngle = (((_totalAngle) / _total) * subParts![i]);
          final paint = Paint();

          final normalizedPrevAngle = (prevAngle! - 0.15) % doublePi;
          ...
          final Gradient gradient = SweepGradient(
            ...
            colors: getGradient(gradientList!, i, isNonGradientElementPresent: isNonGradientElementPresent, emptyColorGradient: emptyColorGradient!),
          );
        ...
        final radians = prevAngle! + (((_totalAngle / _total) * subParts![i]) / 2);
        final x = (radius) * math.cos(radians);
        final y = (radius) * math.sin(radians);

        if (subParts!.elementAt(i) > 0) {
          final value = formatChartValues != null ? formatChartValues!(subParts!.elementAt(i)) : subParts!.elementAt(i).toStringAsFixed(decimalPlaces!);

          if (showChartValues!) {
            final name = showValuesInPercentage == true ? ('${((subParts!.elementAt(i) / _total) * 100).toStringAsFixed(decimalPlaces!)}%') : value;
            // aa lable mate comment karelu 6e
            // _drawName(canvas, name, x, y, side);
          }
        }
        prevAngle = prevAngle! + (((_totalAngle) / _total) * subParts![i]);
      }
    }

    /// Center mate commment karelu 6e
